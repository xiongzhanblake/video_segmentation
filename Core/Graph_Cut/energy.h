...
public:
	...
	typedef typename GraphT::arc_id Edge;
	...
	void Init(){this->maxflow_init();};
	int get_var(Var x);
	void readVar (Var n) {this->readNode(n);};
	void setVarID (Var n, int id){this->setNodeID(n,id);};
/***********************************************************************/
/***********************************************************************/
/***********************************************************************/
...
inline Energy<captype,tcaptype,flowtype>::Energy(void (*err_function)(const char *), bool tf) : GraphT(err_function), truncate_flag(tf)
{
	Econst = 0;
...
template <typename captype, typename tcaptype, typename flowtype> 
inline typename Energy<captype,tcaptype,flowtype>::Var Energy<captype,tcaptype,flowtype>::add_variable() { return this->add_node(); }

template <typename captype, typename tcaptype, typename flowtype> 
inline void Energy<captype,tcaptype,flowtype>::add_constant(Value A) { Econst += A; }

template <typename captype, typename tcaptype, typename flowtype> 
inline void Energy<captype,tcaptype,flowtype>::add_term1(Var x, Value A, Value B) { this->add_tweights(x, B, A); }
...
template <typename captype, typename tcaptype, typename flowtype> 
inline typename Energy<captype,tcaptype,flowtype>::TotalValue Energy<captype,tcaptype,flowtype>::minimize() { return Econst + this->maxflow(); }
